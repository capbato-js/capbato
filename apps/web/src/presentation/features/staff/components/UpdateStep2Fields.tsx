...
interface UpdateStep2FieldsProps {
  ...
  currentSchedulePattern?: string; // Current schedule pattern to exclude from "taken" list
}

export const UpdateStep2Fields: React.FC<UpdateStep2FieldsProps> = ({
  control,
  errors,
  isLoading,
  register,
  onInputChange,
  fieldErrors = {},
  currentSchedulePattern
}) => {
  const { scheduleOptions, loading: scheduleLoading } = useAvailableSchedulePatterns();

  // Filter to show options but allow current schedule pattern even if taken by another doctor
  const schedulePatternData = scheduleOptions.map(option => ({
    value: option.value,
    label: option.available || option.value === currentSchedulePattern
      ? option.label 
      : `${option.label} (Taken by ${option.takenBy})`,
    disabled: !option.available && option.value !== currentSchedulePattern
  }));
