...
describe('LoginForm', () => {
  ...
  it('should show loading spinner when submitting', () => {
    ...
    // Should show loading state on the button (Mantine button has loading prop)
    const submitButton = screen.getByTestId('login-submit-button');
    expect(submitButton).toBeDisabled();
    // The loading state is handled by Mantine internally, so we check if button is in loading state
