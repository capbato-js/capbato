...
export class SqliteLabRequestRepository implements ILabRequestRepository {
  ...
  async save(labRequest: LabRequest): Promise<LabRequest> {
    ...
    const result = stmt.run(
      ...
      tests.hepaBScreening,
      tests.hepaAScreening,
      tests.hepatitisProfile,
      tests.vdrlRpr,
      tests.dengueNs1,
      tests.ca125CeaPsa,
      tests.fbs,
  ...
  async findCompleted(): Promise<LabRequest[]> {
    ...
    const rows = stmt.all('complete') as LabRequestRecord[];
    return rows.map(row => this.recordToDomain(row));
  }
  ...
  private recordToDomain(record: LabRequestRecord): LabRequest {
    ...
    const tests = LabRequestTests.create({
      ...
      hepaBScreening: record.hepaBScreening,
      hepaAScreening: record.hepaAScreening,
      hepatitisProfile: record.hepatitisProfile,
      vdrlRpr: record.vdrlRpr,
      dengueNs1: record.dengueNs1,
      ca125CeaPsa: record.ca125CeaPsa,
      fbs: record.fbs,
      bun: record.bun,
      ...
      sgpt: record.sgpt,
      alp: record.alp,
      sodiumNa: record.sodiumNa,
      potassiumK: record.potassiumK,
      hbalc: record.hbalc,
      ecg: record.ecg,
      t3: record.t3,
      t4: record.t4,
    ...
    const status = LabRequestStatus.create(record.status as 'pending' | 'complete' | 'cancelled');
