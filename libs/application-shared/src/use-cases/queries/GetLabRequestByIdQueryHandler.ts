...
/**
 * Query handler for retrieving a lab request by ID
...
export class GetLabRequestByIdQueryHandler {
  ...
  async execute(id: string): Promise<LabRequest> {
    const labRequest = await this.labRequestRepository.getById(id);
    
    if (!labRequest) {
      throw new LabRequestNotFoundException(id);
    }
